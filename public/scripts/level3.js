...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const questions = await fetch('/api/getQuestions?level=3', {
        method: 'GET',
...
function editQuestion(button) {
    ...
    popup.querySelector('.save-btn').addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`/api/updateQuestion?questionNumber=${questionNumber}&level=3`, {
                method: 'POST',
...
async function deleteQuestion(button) {  
   ...
   try { 
     const deleteQuestResponse= await fetch(`/api/deleteQuestion?questionNumber=${questionNumber}&level=3`, { method:"GET", headers:{ Accept:"application/json" }, }); 
...
async function addNewQuestion() { 
   try { 
       ...
       let addQuestResponse= await fetch("/api/addQuestion?level=3", { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(data), }); 
...
function editQuestion(button) {
   ...
   // console.log(questionNumber)
    ...
    popup.querySelector('.save-btn').addEventListener('click', async() => {
        ...
        const update = await fetch(`/api/updateQuestion?questionNumber=${questionNumber}&level=3`, {
            method: 'POST',
            headers: {
...
async function deleteQuestion(button) {  
    const container = button.closest('.question-container');
    const questionNumber = container.id
    const deleteQuest = await fetch(`/api/deleteQuestion?questionNumber=${questionNumber}&level=3`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json'
        },
...
async function addNewQuestion() {
    ...
    console.log(data);
    const addQuest = await fetch('/api/addQuestion?level=3', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
